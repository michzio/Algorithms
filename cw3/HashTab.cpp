...
class HashTab
{
    ...
    unsigned int Hash(const string & pStr) 
    {
        ...
        //sumujemy w pętli kody ASCII (int) odpowiadajace poszczegolnym znakom ciagu
    ...
    unsigned int Hash_Prime(const string & pStr)
    {
    ...
    unsigned int HashDouble(const string &pStr, int i)
    {
        return (Hash(pStr) + i*Hash_Prime(pStr)) & HASHTAB_MASK;
    }
